...
Standard::Standard(Mediator *pMediator, double radius, double speed, int points) : Bird()
{
   // set the position: standard birds start from the middle
   pt.setY(randomFloat(dimensions.getY() * 0.25, dimensions.getY() * 0.75));
   pt.setX(0.0);
   ...
   this->pBirdColleague = new BirdColleague(this, pMediator);
}
...
{   
   StandardAdvance birdAdvance;
   birdAdvance.advance(this);
}
...
void Sinker::advance()
{
   SinkerAdvance sinkerAdvance;
   sinkerAdvance.advance(this);
}

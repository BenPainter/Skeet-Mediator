...
void Skeet::animate()
{
   ...
   for (auto it = birds.begin(); it != birds.end();)
      if ((*it)->isDead())
      {
         ...
         mediator.removeColleague((*it)->pBirdColleague);
         it = birds.erase(it);
      }
      else
         ++it;
   ...
   for (auto it = bullets.begin(); it != bullets.end(); )
      if ((*it)->isDead())
      {
         mediator.removeColleague((*it)->pBulletColleague);
         (*it)->death(bullets);
         it = bullets.erase(it);
      }
      else
...
void Skeet::interact(const UserInput & ui)
{
   ...
   if (ui.isSpace())
      p = bulletFactory(&mediator, Bullet::PELLET, gun.getAngle());
   else if (ui.isM() && time.level() > 1)
      p = bulletFactory(&mediator, Bullet::MISSILE, gun.getAngle());
   else if (ui.isB() && time.level() > 2)
      p = bulletFactory(&mediator, Bullet::BOMB, gun.getAngle());
   ...
   {
      bullets.push_back(p);
      mediator.addColleague(p->pBulletColleague);
...
void Skeet::spawn()
{
   ...
   switch (time.level())
   {
      ...
      case 1:
         ...
         {
            Bird *bird = new Standard(&mediator, size, 7.0);
            birds.push_back(bird);
            mediator.addColleague(bird->pBirdColleague);
            std::cout << mediator.colleagues.size() << std::endl;
         }
            
         // spawn every 4 seconds
         if (random(0, 4 * 30) == 1)
         {
            Bird* bird = new Standard(size, 7.0);
            birds.push_back(bird);
            mediator.addColleague(bird->pBirdColleague);
         }
         break;
      ...
      case 2:
         ...
         if (birds.size() == 0 && random(0, 15) == 1)
            {
               Bird *bird = new Standard(size, 7.0, 12);
               birds.push_back(bird);
               mediator.addColleague(bird->pBirdColleague);
            }
         ...
         if (random(0, 4 * 30) == 1)
         {
            Bird *bird = new Standard(size, 5.0, 12);
            birds.push_back(bird);
            mediator.addColleague(bird->pBirdColleague);
         }
      ...
      case 3:
         ...
         if (birds.size() == 0 && random(0, 15) == 1)
         {
            Bird *bird = new Standard(size, 5.0, 15);
            birds.push_back(bird);
            mediator.addColleague(bird->pBirdColleague);
         }
         ...
         if (random(0, 4 * 30) == 1)
          {
            Bird *bird = new Sinker(size, 4.0, 22);
            birds.push_back(bird);
            mediator.addColleague(bird->pBirdColleague);
         }
         // spawn every 4 seconds
      ...
      case 4:
         ...
         if (birds.size() == 0 && random(0, 15) == 1)
         {
            Bird *bird = new Standard(size, 4.0, 18);
            birds.push_back(bird);
            mediator.addColleague(bird->pBirdColleague);
         }
         ...
         if (random(0, 4 * 30) == 1)
         {
            Bird *bird = new Sinker(size, 3.5, 25);
            birds.push_back(bird);
            mediator.addColleague(bird->pBirdColleague);
         }
         // spawn every 4 seconds
         if (random(0, 4 * 30) == 1)
         {
            Bird *bird = new Floater(size, 4.0, 25);
            birds.push_back(bird);
            mediator.addColleague(bird->pBirdColleague);
         }
         // spawn every 4 seconds
